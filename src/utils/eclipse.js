...
// Latitude to text
export const latitudeToString = latitude => {
  const latt = Math.round(latitude * 10000) / 10000.0
...
// Longitude to text
export const longitudeToString = longitude => {
  const longt = Math.round(longitude * 10000) / 10000.0
...
export const loc_circ = (lat, lon, newElements) => {
  ...
  return {
    ...
    eclipse: mid[36],
    maxmag: maxmag || 0,
    coverage: isEclipse ? getcoverage() : 0,
    duration: isEclipse ? getduration() : 0,
    c1: {
      alt: getalt(c1),
      azi: getazi(c1),
      date: convertToDateTime(c1),
    },
    c2: {
      alt: getalt(c2),
      azi: getazi(c2),
      date: convertToDateTime(c2),
    },
    mid: {
      alt: getalt(mid),
      azi: getazi(mid),
      date: convertToDateTime(mid),
    },
    c3: {
      alt: getalt(c3),
      azi: getazi(c3),
      date: convertToDateTime(c3),
    },
    c4: {
      alt: getalt(c4),
      azi: getazi(c4),
      date: convertToDateTime(c4),
    },
  }
}